...
class Helper {
   ...
   static func monthOfTheRecord(date: Date) -> String {
    ...
    static func RainInUserUnitFromMM(rainMM: Double, userRainUnits: RainfallUnit) -> Double {
        
        var rainInUserUnit:Double = 0.0 
        
        if userRainUnits.text == "mm" {
            rainInUserUnit = rainMM
        }
        
        if userRainUnits.text == "inches" {
            rainInUserUnit = rainMM * 0.0393700787402 // convert from mm to inches
        }
        
        return rainInUserUnit
    }
   ...
   /* static func PastFiveYears(baseYear:Int) -> [Int] {
        
        // creates an array of years from the  baseyear - 5 to baseyear - 1
    ...
    static func PreviousMonth() -> String {
        let requiredDate = AddOrSubtractMonth(month: -1)
        let dateFormatter = DateFormatter()
    ...
    static func PreviousMonthInt() -> Int {
        let requiredDate = AddOrSubtractMonth(month: -1)
        let calendar = Calendar.current
        let components = calendar.dateComponents([.month], from: requiredDate)
        return components.month!
    }
    
    static func PreviousMonthsYear() -> Int {
